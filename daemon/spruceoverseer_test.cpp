...
void SpruceOverseerTest::test( SpruceOverseer *o, Engine *engine )
{
    ...
    /// imitate ticker
    engine->market_info[ TEST_MARKET ].spread.bid = "0.00010000";
    engine->market_info[ TEST_MARKET ].spread.ask = "0.00010100";
    ...
    const Spread regular_spread = o->getSpreadLimit( TEST_MARKET, true );
    ...
    const Spread expanded_spread = o->getSpreadLimit( TEST_MARKET, true );
    ...
    const Spread buy_spread = o->getSpreadForSide( TEST_MARKET, SIDE_BUY, true, false );
    const Spread sell_spread = o->getSpreadForSide( TEST_MARKET, SIDE_SELL, true, false );
    ...
    const Spread buy_spread_random = o->getSpreadForSide( TEST_MARKET, SIDE_BUY, true, false, true, true );
    const Spread sell_spread_random = o->getSpreadForSide( TEST_MARKET, SIDE_SELL, true, false, true, true );
    ...
    const Spread buy_spread_contracted = o->getSpreadForSide( TEST_MARKET, SIDE_BUY, true, false, false, false, Coin( "0.01" ) );
    const Spread sell_spread_contracted = o->getSpreadForSide( TEST_MARKET, SIDE_SELL, true, false, false, false, Coin( "0.01" ) );
    ...
    const Spread midspread_test = o->getSpreadLimit( TEST_MARKET, false );
    assert( midspread_test.bid == midspread_test.ask );

    /// ensure getSpreadLimit() with taker enabled gives us crossed prices
    const Spread taker_spread_buy = o->getSpreadForSide( TEST_MARKET, SIDE_BUY, true, true );
    const Spread taker_spread_sell = o->getSpreadForSide( TEST_MARKET, SIDE_SELL, true, true );
    ...
    const Spread taker_spread_buy_rand = o->getSpreadForSide( TEST_MARKET, SIDE_BUY, true, true, true );
    const Spread taker_spread_sell_rand = o->getSpreadForSide( TEST_MARKET, SIDE_SELL, true, true, true );

    assert( taker_spread_buy_rand.bid > taker_spread_buy_rand.ask );
    assert( taker_spread_sell_rand.bid > taker_spread_sell_rand.ask );

    /// test spread limit
    const Spread spreadlimit = o->getSpreadLimit( TEST_MARKET, true );

    // ensure spread limit is better than our ticker prices
    assert( spreadlimit.bid < engine->market_info[ TEST_MARKET ].spread.bid &&
            spreadlimit.ask > engine->market_info[ TEST_MARKET ].spread.ask );
