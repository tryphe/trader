...
# check if keydefs exists before we create res/session.token
if [ ! -f daemon/keydefs.h ]; then
  failureHints+='token FAILED.(keydefs.h does not exist) '
else
  ...
  if [ -f res/session.token ]; then
    ...
    failureHints+='token FAILED.(failed to create session token) '
  fi
fi
